...
def create_faculty_delete_window(parent, token):
    delete_window = tk.Toplevel(parent)
    delete_window.title("Delete Faculty")
    ...
    title_label = tk.Label(main_frame, text="🧑‍🏫 Delete Faculty", font=("Helvetica", 22, "bold"),
                           bg="#F5E6E0", fg="#333333")
    ...
    faculty_entry = ModernEntry(search_frame, "🆔 Faculty ID")
    faculty_entry.pack(side="left", expand=True, padx=(0, 10))

    search_button = tk.Button(search_frame, text="Search 🔍", command=lambda: search_faculty(),
    ...
    def search_faculty():
        faculty_id = faculty_entry.get_value().strip()
        if not faculty_id:
            status_label.config(text="⚠️ Please enter a faculty ID", fg="#f44336")
            return
    ...
    def delete_faculty():
        ...
        try:
            response = requests.delete(
                f"http://localhost:8080/api/faculty/admin/delete/{faculty_id}",
                headers={
                    'Content-Type': 'application/json',
                    'Authorization': f'Bearer {token}'
                }
    ...
    delete_button = tk.Button(button_frame, text="🗑️ Delete", command=delete_faculty,
